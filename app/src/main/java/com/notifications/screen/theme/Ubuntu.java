...
public class Ubuntu extends ThemeClass {
    ...
    public Ubuntu(ViewStub stub) {
        super(stub);
        stub.setLayoutResource(R.layout.activity_read_inner_ubuntu);


/*

        if (OverlayServiceCommon.preferences.getInt("pro_color", -1)!=-1)
        {
            mDrawable = stub.getContext().getResources().getDrawable(R.drawable.card_ubuntu);

            if (mDrawable==null)
                Mlog.d(logTag, " NULLLLL 2");
            else
                Mlog.d(logTag, " NULLLLL 3");

            mDrawable.setColorFilter(new PorterDuffColorFilter(OverlayServiceCommon.preferences.getInt("pro_color", -1), PorterDuff.Mode.MULTIPLY));
